...
def match_permutations(string, words):
    ...
    matching_perms = sorted(list(set([''.join(p) for p in permutations(string)]) & words))
...
def plur2sing(singular, plural):
    ...
    # inversion using zip
    pl2sg = dict(zip(sg2pl.values(), sg2pl.keys()))
    return pl2sg

def vect2word(word2vect):
    v2w = {tuple(v): k for k, v in word2vect.items()}
    return v2w
